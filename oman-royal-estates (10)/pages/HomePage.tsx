...
import { MOCK_PROPERTIES, getHomePageSettings } from '../data';
import { MAX_FEATURED_PROPERTIES, APP_NAME, COMMON_TRANSLATIONS } from '../constants';
import { Link } from 'react-router-dom';
import { ROUTES } from '../constants';
import { useLanguage } from '../contexts/LanguageContext';
import { SearchIcon, CloseIcon } from '../components/IconComponents'; // Added CloseIcon for drawer
...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      const featured = MOCK_PROPERTIES.filter(p => p.isFeatured).slice(0, MAX_FEATURED_PROPERTIES);
      setFeaturedProperties(featured);
      setHeroSettings(getHomePageSettings());
      setIsLoading(false);
    ...
    <div className="min-h-screen">
      ...
      <section className="relative bg-royal-blue text-white py-20 md:py-32 h-[auto] min-h-[60vh] md:min-h-[70vh] flex flex-col justify-center items-center">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          {/* Desktop Search Bar: Remains stacked due to SearchBar's default !inline behavior */}
          ...
          <div className="h-full flex flex-col">
              <div className="flex justify-between items-center p-4 border-b border-medium-gray bg-white">
                  ...
                  <button 
                    ...
                    aria-label={T.hideFilters} // Re-use "Hide Filters" or add "Close Search"
                  >
                      <CloseIcon className="w-6 h-6" />
                  </button>
              </div>
              <div className="overflow-y-auto flex-grow p-1"> {/* Reduced padding around SearchBar container */}
                {/* SearchBar itself has padding, so this container can be tighter */}
