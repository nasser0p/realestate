...
import { MOCK_PROPERTIES, CITIES, AMENITIES, addMockProperty, updateMockProperty, MOCK_TEAM_MEMBERS } from '../data';
import { Property, PropertyStatus, PropertyType, Amenity as AmenityType, City as CityType, TeamMember } from '../types';
...
const AdminEditPropertyPage: React.FC = () => {
  ...
  // New state for agent selection dropdown
  ...
  useEffect(() => {
    ...
    if (isNew) {
      ...
      setSelectedAgentId(''); // Reset agent dropdown
      setIsLoading(false);
    } else {
  ...
  const handleAgentSelectionChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // If agentId is empty (e.g., "--- Select ---" chosen), current agent details remain,
    // allowing manual input or retaining previously set details.
  };
