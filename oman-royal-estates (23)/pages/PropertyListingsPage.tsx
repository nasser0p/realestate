...
import { collection, getDocs, query, orderBy, Timestamp, QuerySnapshot, DocumentData } from 'firebase/firestore';
...
const PropertyListingsPage: React.FC = () => {
        ...
        try {
            ...
            const propsList = querySnapshot.docs.map(docSnap => {
                ...
                return {
                    id: docSnap.id,
                    ...data,
                    // Convert Firestore Timestamps to JS Date objects or ISO strings if needed for display/logic
                    // For now, PropertyCard and other components might need to handle Timestamp objects.
                    // Or convert here:
            ...
            setAllProperties([]); // Set to empty on error
        }
        // setIsLoading(false); // Loading will be set to false after filtering
    };
    fetchAllProperties();
  }, []);
  ...
  useEffect(() => {
    if (allProperties.length === 0 && !isLoading) { // If allProperties is empty but not due to initial load
        setFilteredProperties([]);
        return;
    }
    if (allProperties.length === 0 && isLoading && location.search === "") return; // Still initial loading of all props
    ...
    setCurrentPage(1);
    setIsLoading(false); // Finish loading after filtering
  }, [allProperties, location.search, parseQueryFilters, language, isLoading]);
