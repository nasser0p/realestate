...
import { CITIES, AMENITIES } from '../data'; // MOCK_TEAM_MEMBERS removed
...
import { doc, getDoc, setDoc, addDoc, collection, Timestamp, serverTimestamp, updateDoc, query, orderBy, getDocs as getFirestoreDocs } from 'firebase/firestore';
...
const AdminEditPropertyPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      try {
        ...
        const q = query(teamColRef, orderBy('name', 'asc'));
        const teamSnapshot = await getFirestoreDocs(q);
        const membersList = teamSnapshot.docs.map(docSnap => ({
          id: docSnap.id,
          ...docSnap.data()
        } as TeamMember));
        setTeamMembersList(membersList);
        return membersList; // Return for immediate use in loadProperty
      } catch (error) {
        console.error("Error fetching team members:", error);
        setTeamMembersList([]);
        return []; // Return empty for immediate use in loadProperty
      }
    };

    const loadProperty = async (currentTeamMembers: TeamMember[]) => {
      setIsLoading(true);
      ...
      if (isNew) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            // Set agent details from property first
            ...
            // Try to pre-select agent from team list
            if (foundProperty.agent) {
                const { name, name_ar, email, phone } = foundProperty.agent;
                const matchedAgent = currentTeamMembers.find(tm =>
                    tm.name === name &&
                    (name_ar ? tm.name_ar === name_ar : tm.name_ar === '' || !tm.name_ar) && // Match if property has name_ar or team member has empty/no name_ar
                    tm.email === email &&
                    tm.phone === phone
                );
                if (matchedAgent) {
    ...
    fetchTeamMembers().then(currentTeamMembers => {
        loadProperty(currentTeamMembers);
    });
  ...
  const handleAgentSelectionChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (agentId) {
      const selectedMember = teamMembersList.find(member => member.id === agentId);
      if (selectedMember) {
  ...
  const handleManualAgentInputChange = <K extends keyof Pick<TeamMember, 'name' | 'name_ar' | 'email' | 'phone'>>(
    setter: React.Dispatch<React.SetStateAction<string>>,
    value: string
  ) => {
    setter(value);
    setSelectedAgentId(''); // Clear dropdown selection if manual edit
  };
  ...
  const removeGalleryImage = async (idToRemove: string) => {
    ...
    setGalleryItems(prev => prev.filter(item => {
        if (item.id === idToRemove && item.file) { 
    ...
    if (floorPlanItem) {
        if (floorPlanItem.storagePath && !floorPlanItem.isNewlyAdded) {
            try {
                ...
                setFloorPlanError(T.imageUploadError); 
            }
        }
        if (floorPlanItem.file) { 
    ...
    try {
        ...
        for (const item of galleryItems) {
            if (item.isNewlyAdded && item.file) {
                ...
                uploadedGalleryUrls.push(item.previewUrl); 
        ...
        const propertyDataToSave: Omit<Property, 'id'> = {
            ...
            dateAdded: initialPropertyData?.dateAdded || serverTimestamp(), 
        ...
        if (isNew) {
            ...
            alert(T.propertyCreatedSuccess.replace(' (Client-side mock)', ''));
        } else if (initialPropertyData) {
            await updateDoc(doc(db, 'properties', currentPropertyId), {
                ...propertyDataToSave,
                dateAdded: initialPropertyData.dateAdded, 
                createdAt: initialPropertyData.createdAt 
            });
            alert(T.propertyUpdatedSuccess.replace(' (Client-side mock)', ''));
        }
        ...
        <fieldset className="border border-medium-gray p-4 rounded-md">
            ...
            <div className="mb-4">
              ...
              <select
                ...
                disabled={isSaving || isGeneratingEn || isGeneratingAr || teamMembersList.length === 0}
              >
                <option value="">{T.selectAgentPrompt}</option>
                {teamMembersList.map((member: TeamMember) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="text" id="agentName" value={agentName} 
                           onChange={(e) => handleManualAgentInputChange(setAgentName, e.target.value)}
                           className={inputClass} disabled={isSaving || isGeneratingEn || isGeneratingAr} />
                </div>
                 <div>
                    <label htmlFor="agentNameAr" className={labelClass}>{T.agentNameLabel} (AR)</label>
                    <input type="text" id="agentNameAr" value={agentNameAr} 
                           onChange={(e) => handleManualAgentInputChange(setAgentNameAr, e.target.value)}
                           className={inputClass} disabled={isSaving || isGeneratingEn || isGeneratingAr} dir="rtl"/>
                </div>
                <div>
                    <label htmlFor="agentEmail" className={labelClass}>{T.agentEmailLabel}</label>
                    <input type="email" id="agentEmail" value={agentEmail} 
                           onChange={(e) => handleManualAgentInputChange(setAgentEmail, e.target.value)}
                           className={inputClass} disabled={isSaving || isGeneratingEn || isGeneratingAr} />
                </div>
                <div>
                    <label htmlFor="agentPhone" className={labelClass}>{T.agentPhoneLabel}</label>
                    <input type="tel" id="agentPhone" value={agentPhone} 
                           onChange={(e) => handleManualAgentInputChange(setAgentPhone, e.target.value)}
                           className={inputClass} disabled={isSaving || isGeneratingEn || isGeneratingAr} />
                </div>
            </div>
        </fieldset>
